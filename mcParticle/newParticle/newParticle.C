...
const Foam::label Foam::newParticle::maxNBehind_ = 10;

Foam::label Foam::newParticle::particleCount_ = 0;

namespace Foam
{
    defineTypeNameAndDebug(newParticle, 0);
}


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::newParticle::stationaryTetReverseTransform
(
    vector& centre,
    scalar& detA,
    barycentricTensor& T
...
void Foam::newParticle::movingTetReverseTransform
(
...
void Foam::newParticle::reflect()
{
    Swap(coordinates_.c(), coordinates_.d());
}


void Foam::newParticle::rotate(const bool reverse)
{
    if (!reverse)
    {
...
void Foam::newParticle::changeTet(const label tetTriI)
{
    if (debug)
    {
...
void Foam::newParticle::changeFace(const label tetTriI)
{
    if (debug)
    {
        Info << "Particle " << origId() << nl << FUNCTION_NAME << nl << endl;
...
void Foam::newParticle::changeCell()
{
    if (debug)
    {
        Info << "Particle " << origId() << nl << FUNCTION_NAME << nl << endl;
...
void Foam::newParticle::changeToMasterPatch()
{
    if (debug)
    {
        Info << "Particle " << origId() << nl << FUNCTION_NAME << nl << endl;
...
)
{
    ...
    {
        FatalErrorInFunction
            << "Cell not found for newParticle position " << position << "."
            << exit(FatalError);
    }
    celli_ = celli;
    ...
    // The newParticle must be (hopefully only slightly) outside the cell. Track
    ...
    else
    {
        ...
        {
            WarningInFunction
                << "Suppressing any further warnings about newParticles being "
...
Foam::newParticle::newParticle(const newParticle& p)
:
    mesh_(p.mesh_),
...
Foam::newParticle::newParticle(const newParticle& p, const polyMesh& mesh)
:
    mesh_(mesh),
    coordinates_(p.coordinates_),
    celli_(p.celli_),
...
Foam::scalar Foam::newParticle::track
(
...
Foam::scalar Foam::newParticle::trackToCell
(
    const vector& displacement,
    const scalar fraction
)
...
Foam::scalar Foam::newParticle::trackToFace
(
    const vector& displacement,
    const scalar fraction
)
...
Foam::scalar Foam::newParticle::trackToStationaryTri
(
    const vector& displacement,
    const scalar fraction,
    label& tetTriI
...
Foam::scalar Foam::newParticle::trackToMovingTri
(
    const vector& displacement,
    const scalar fraction,
    label& tetTriI
)
{
    ...
    // !!! <-- This fails if the tet collapses onto the newParticle, as detA tends
    ...
    {
        FatalErrorInFunction
            << "A moving tet collapsed onto a newParticle. This is not supported. "
            << "The mesh is too poor, or the motion too severe, for newParticle "
...
Foam::scalar Foam::newParticle::trackToTri
(
    const vector& displacement,
    const scalar fraction,
    label& tetTriI
...
Foam::vector Foam::newParticle::deviationFromMeshCentre() const
{
...
void Foam::newParticle::transformProperties(const tensor&)
{}


void Foam::newParticle::transformProperties(const vector&)
{}


void Foam::newParticle::prepareForParallelTransfer()
{
...
void Foam::newParticle::correctAfterParallelTransfer
(
    const label patchi,
    trackingData& td
)
...
void Foam::newParticle::prepareForInteractionListReferral
(
...
void Foam::newParticle::correctAfterInteractionListReferral(const label celli)
{
    ...
    // position. This isn't likely to be correct; the newParticle is probably not
    // in this tet. It will, however, generate the correct vector when the
    // position method is called. A referred newParticle should never be tracked,
...
Foam::label Foam::newParticle::procTetPt
(
...
) const
{
    ...
    // same as in newParticle::correctAfterParallelTransfer.
...
bool Foam::operator==(const newParticle& pA, const newParticle& pB)
{
